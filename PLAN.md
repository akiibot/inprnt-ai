...
1. NEVER put text in the background prompt. The image model cannot render 
   text. Add "no text no letters no words no writing" to every background prompt.
...
3. Position keywords must be one of:
   "top-left", "top-center", "top-right",
   "center-left", "center", "center-right",
   "bottom-left", "bottom-center", "bottom-right"

4. Font families must come from the brand identity. Use:
...
6. If language is "both", include separate Bengali and English text layers.
   Bengali text goes in a layer with font_family set to the _bn font.
...
10. If product_image_available is true, include a product-hero layer 
    ...
    Place it between the overlay and the text layers in z-index.
...
12. font_size values must be appropriate for the format dimensions.
